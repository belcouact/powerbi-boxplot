...
export class Visual implements IVisual {
    ...
    private processData(dataView: DataView, width: number, height: number): BoxplotData[] {
        ...
        const useSingleGroup = !hasExplicitCategory && valuesIdx !== -1;
        if (categoryIdx === -1 && !useSingleGroup) {
            categoryIdx = 0;
        }
        ...
        if (validCount > 0 && groupedData.size > 0) {
            ...
            if (detailIdx === -1 && totalGroupValues < table.rows.length * 0.5) {
                this.showDebugMessage(width, height,
                    `Warning: Only ${totalGroupValues} of ${table.rows.length} rows produced numeric values.\n` +
                    `If your Values field contains few unique numbers, Power BI may be deduplicating rows.\n\n` +
                    `Fix: Drag a unique identifier (e.g. Sample Number) into the "Detail" field\n` +
                    `to preserve all individual data points.`);
                return [];
            }
        }
